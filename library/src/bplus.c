...
void recursive_insert(btree_node node, TS student, const char* index_btree, const char* leaf_btree) {
    if (node.is_leaf) {
        ...
        read_node(index_btree, node.children[i], child);
        child.parent_pointer = node.position;
        write_node(index_btree, child);
        recursive_insert(child, student, index_btree, leaf_btree);
    }
}
...
void testing_btree(FILE *file_student){
    ...
    for(int i = 0; i < 10000; i++){
        ...
        j++;
        root_position = btree_insert(root_position, student, "index_btree.bin", "leaf_btree.bin");
    ...
    int verify = 0;
    for (int i = 0; i < j; i++) {
            long long int result = search_cpf("leaf_btree.bin", vector[i], root_position);
            if (result != -1) {
                printf("%d: found: %lld\n", i + 1, result);
                verify++;
            }
    ...
    for(int i = 0; i < 100; i++){
        ...
        root_position = btree_remove(root_position, vector[random_position], "ArqIdx.bin");
    ...
    for (int i = 0; i < j; i++) {
        if (search_cpf("ArqIdx.bin", vector[i], root_position) != -1) {
            verify++;
        } else {
            printf("O cpf %lld não foi encontrado, ma deveria existir.\n", vector[i]);
        }
    }

    for (int i = 0; i < 100; i++) {
        if (search_cpf("ArqIdx.bin", cpfs_to_remove[i], root_position) == -1) {
            not_found++;
        } else {
